...
public:
	...
	BattleshipGame()
    {
		...
		for (int i = 0; i<5; i++) {
			allplayerSub[i]=Submarine();
			allcomputerSub[i]=Submarine();
		}
       // playerSub = Submarine();
    ...
    void placePlayerPieces(int i)
    {
        ...
        do{
            displayBoards();
            correctPlacement = placePiece(allplayerSub[i]);
            if(!correctPlacement)
            {
        ...
        addPieceToBoard(playerBoard, allplayerSub[i]);
    }
    ...
    void placeComputerPieces(string s)
    {
        ...
        for(int i = 0; i <5 ; i++) {
			do{
				...
				allcomputerSub[i].setPiece(randomRow, randomColumn, getFirstChar(randomDirection));
				correctPlacement = isValidPlacement(tempBoard, allcomputerSub[i]);
			}while(!correctPlacement);

			addPieceToBoard(tempBoard, allcomputerSub[i]);
			in.close();
		}
    }
    ...
    string playerMove()
    {
        ...
        for(int i=0; i<5; i++) {
			...
			int row = allcomputerSub[i].getTopLeftRow();
			int column = allcomputerSub[i].getTopLeftColumn();
			for(int j = 0; j < allcomputerSub[i].getLength(); j++)
			{
				if(row == attackRow && column == attackColumn)
				{
					computerBoard[attackRow][attackColumn] = HIT;
					if(isSunk(computerBoard,allcomputerSub[i]))
					{
						string result = allcomputerSub[i].getName();
						result +=" sunk at ";
				...
				if(allcomputerSub[i].isHorizontal())
				{
					column++;
				}
				else
    ...
    string computerMove()
    {
		...
		for(int i =0; i<5; i++) {
			...
			int row = allplayerSub[i].getTopLeftRow();
			int column = allplayerSub[i].getTopLeftColumn();
			for(int j = 0; j < allplayerSub[i].getLength(); j++)
			{
				if(row == attackRow && column == attackColumn)
				{
					playerBoard[attackRow][attackColumn] = HIT;
					if(isSunk(playerBoard,allplayerSub[i]))
					{
						string result = allplayerSub[i].getName();
						result += " sunk at ";
						result += coordinate;
						return result;
					}
				...
				if(allplayerSub[i].isHorizontal())
				{
					column++;
				}
				else
    ...
    int gameOver()
    {
        ...
        for(int j = 0; j<5; j++) {
			computerShipCoordinateTotal += allcomputerSub[j].getLength();

			
			playerShipCoordinateTotal += allplayerSub[j].getLength();
...
private:
    ...
    Submarine allplayerSub[5];
    Submarine allcomputerSub[5];
...
int main()
{
	...
	game.placeComputerPieces(""); // can input a file, but we not using
	
	// place your pieces
	for (int i=0;i<5;i++) {
		
		game.placePlayerPieces(i);
	}
